...
INSERT INTO tipo_costo_fijo (codigo, nombre) VALUES
...
FROM (
         SELECT periodo, 'ELECTRICIDAD' codigo, COALESCE(electricidad,0) monto FROM costosfijos
         UNION ALL SELECT periodo, 'AFIP', COALESCE(afip,0) FROM costosfijos
         UNION ALL SELECT periodo, 'MUNICIPALIDAD', COALESCE(municipalidad,0) FROM costosfijos
         UNION ALL SELECT periodo, 'RENTAS', COALESCE(rentas,0) FROM costosfijos
         UNION ALL SELECT periodo, 'ALQUILER', COALESCE(alquiler,0) FROM costosfijos
         UNION ALL SELECT periodo, 'EXPENSAS', COALESCE(expensas,0) FROM costosfijos
         UNION ALL SELECT periodo, 'SUELDO', COALESCE(sueldo,0) FROM costosfijos
         UNION ALL SELECT periodo, 'AGUA', COALESCE(agua,0) FROM costosfijos
         UNION ALL SELECT periodo, 'INGRESOS_BRUTOS', COALESCE(ingresosbrutos,0) FROM costosfijos
         UNION ALL SELECT periodo, 'PLUS_SUELDOS', COALESCE(plussueldos,0) FROM costosfijos
         UNION ALL SELECT periodo, 'TELEFONO', COALESCE(telefono,0) FROM costosfijos
         UNION ALL SELECT periodo, 'CABLE_INTERNET', COALESCE(cable_internet,0) FROM costosfijos
         UNION ALL SELECT periodo, 'EMPLEADO', COALESCE(empleado,0) FROM costosfijos
         UNION ALL SELECT periodo, 'TARJETA', COALESCE(tarjeta,0) FROM costosfijos
     ) x
